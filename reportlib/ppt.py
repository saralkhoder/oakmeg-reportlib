"""Module for generating PowerPoint elements like tables"""
...
def save_as_table(df: pd.DataFrame, to: str):
    ...
    """
    rows, cols = df.shape
    assert rows < 30 and cols < 10, 'Table is too large ! (more than 30 rows or 10 columns)'
    ...
    for r in range(rows):
        for c in range(cols):
            table.cell(r + 1, c).text = str(df.iloc[r, c]) if isinstance(df.iloc[r, c], str) else f'{df.iloc[r, c]:,}'
